	...
	private void Update() {
		...
		if (state == 3) //This state means this unit is capable of attacking it's target
		{
			...
			if (EnemyManager.instance.GetUnit(target) == null) {
				state = 0;
				target = -1;
			} else if (Vector3.Distance(transform.position, EnemyManager.instance.GetUnit(target).transform.position) >= 5f) //if target moves too far away, start moving towards them
			  {
				state = 2;
				base.SetDestination(EnemyManager.instance.GetUnit(target).transform.position);
